...
cp ${HOLOGRAM_DIR}/config/agent.json /hologram-build/darwin/root/etc/hologram/agent.json
cp ${HOLOGRAM_DIR}/agent/support/darwin/com.adroll.hologram* /hologram-build/darwin/root/Library/LaunchDaemons/
cp ${HOLOGRAM_DIR}/agent/support/darwin/postinstall.sh /hologram-build/darwin/scripts/postinstall
...
mkbom -u 0 -g 80 /hologram-build/darwin/root /hologram-build/darwin/flat/base.pkg/Bom || exit 1
( cd /hologram-build/darwin/flat/base.pkg && /usr/local/bin/xar --compression none -cf "${HOLOGRAM_DIR}/artifacts/Hologram-${GIT_TAG}.pkg" * ) || exit 1
